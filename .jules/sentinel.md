...
## 2024-08-20 - Unencrypted Data Transmission for OUI Database
**Vulnerability:** The `download_oui_db()` function fetched the official IEEE OUI registry over an insecure HTTP connection (`http://standards-oui.ieee.org/oui/oui.txt`).
**Learning:** Downloading files, especially security-relevant or data-integrity-dependent files like registries, over plaintext HTTP allows a Man-in-the-Middle (MitM) attacker to intercept and modify the payload in transit. This could lead to incorrect OUI lookups or spoofed vendor data being ingested by the application.
